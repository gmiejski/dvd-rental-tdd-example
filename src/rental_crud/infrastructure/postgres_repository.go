...
func (p *postgresRepository) Save(rents rental_crud.UserRents) error {
...
func (p *postgresRepository) Get(userID int) (*rental_crud.UserRents, error) {
	...
	return &rental_crud.UserRents{UserID: userID, RentedMovies: rentedMovies}, nil
}

func (p *postgresRepository) toRentedMovies(rows *sql.Rows) ([]rental_crud.RentedMovie, error) {
	var movies []rental_crud.RentedMovie
	defer rows.Close()
	for rows.Next() {
		var movie = rental_crud.RentedMovie{}
		err := rows.Scan(
...
func NewPostgresRepository(dbDSN string) rental_crud.Repository {
